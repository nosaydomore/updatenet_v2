...
from utils.utils_rpn import get_subwindow_tracking
...
def SiamRPN_init(im, target_pos, target_sz, net):
    ...
    state['gt_z_f'] = z_f.cpu().data
    return state


def SiamRPN_track(state, im, gt_pos, gt_sz):
    p = state['p']
    net = state['net']
    ...
    if gt_sz[0]*gt_sz[1]>0:
        gt_wc_z = gt_sz[0] + p.context_amount * sum(gt_sz)
        gt_hc_z = gt_sz[1] + p.context_amount * sum(gt_sz)
        gt_s_z = round(np.sqrt(gt_wc_z * gt_hc_z))
        # initialize the exemplar


        gt_z_crop = get_subwindow_tracking(im, gt_pos, p.exemplar_size, gt_s_z, avg_chans)
        gt_z = Variable(gt_z_crop.unsqueeze(0))
        gt_z_f = net.featextract(gt_z.cuda()).cpu().data
    else:
        gt_z_f=torch.zeros([1, 512, 6, 6], dtype=torch.float32)

    state['z_f'] = state['z_0']  # acculmate_template, in init RPN phrase, do not update,so it is init template.
    state['z_f_cur'] = z_f.cpu().data  # current_template
    state['gt_z_f'] = gt_z_f  # gt_template (locate on gt_pos and sz)
    state['net'] = net
    state['target_pos'] = target_pos
    state['target_sz'] = target_sz
    state['score'] = score
