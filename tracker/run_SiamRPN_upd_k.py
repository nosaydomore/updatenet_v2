...
def SiamRPN_track_upd(state, im, updatenet):
    ...
    z_f_cur = net.featextract(z_crop.cuda())

    temp = torch.cat((Variable(state['z_0']).cuda(), Variable(state['z_f']).cuda(), z_f_cur), 1)
    init_inp = Variable(state['z_0']).cuda()

    z_f_upd = updatenet(temp, init_inp)
    net.kernel(z_f_upd)


    state['z_f'] = z_f_upd.cpu().data
    state['z_f_cur'] = z_f_cur.cpu().data
    state['net'] = net
    state['target_pos'] = target_pos
    state['target_sz'] = target_sz
    state['score'] = score
